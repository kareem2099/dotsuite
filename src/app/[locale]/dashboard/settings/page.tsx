  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-16">
            ...
            {[
              ...
              <div key={key} className="flex items-center justify-between p-4 bg-(--background) border border-(--card-border) rounded-lg">
                ...
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${notifications[key as keyof typeof notifications] ? "bg-(--primary)" : "bg-(--card-border)"
                    }`}
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-5 h-5 bg-(--toggle-knob) rounded-full shadow transition-transform ${notifications[key as keyof typeof notifications] ? "translate-x-5" : "translate-x-0"
                      }`}
                  />
                </button>
              </div>
      ...
      {activeTab === "privacy" && (
        <div className="space-y-6">
              ...
              {[
                ...
                <div key={key} className="flex items-center justify-between p-4 bg-(--background) border border-(--card-border) rounded-lg">
                  ...
                  <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition-colors ${privacy[key as keyof typeof privacy] ? "bg-(--primary)" : "bg-(--card-border)"
                      }`}
                  >
                    <span
                      className={`absolute top-0.5 left-0.5 w-5 h-5 bg-(--toggle-knob) rounded-full shadow transition-transform ${privacy[key as keyof typeof privacy] ? "translate-x-5" : "translate-x-0"
                        }`}
                    />
                  </button>
                </div>
          ...
          <div className="p-8 bg-(--card-bg) border border-(--danger-border) rounded-xl">
            <h3 className="text-lg font-semibold mb-2 text-(--danger)">{t("dangerZone")}</h3>
            <p className="text-(--text-muted) text-sm mb-4">
              {t("dangerZoneDesc")}
            </p>
            <button
              onClick={() => setShowDeleteModal(true)}
              className="px-4 py-2 border border-(--danger-border) text-(--danger) rounded-lg text-sm hover:bg-(--danger-bg) transition-colors">
              {t("deleteAccount")}
            </button>
          </div>
          <DeleteAccountModal
      ...
      <div className="mt-8 flex justify-end">
        <button
          onClick={handleSave}
          className="flex items-center gap-2 px-6 py-3 bg-(--primary) text-(--primary-text) font-semibold rounded-lg hover:bg-(--primary-hover) transition-colors"
        >
